...
if __name__ == '__main__':
    ...
    while last_bookmark_time_ymdh_str <= current_time_ymdh_str:
        ...
        # next_s3_path = f's3a://chiholee-datalake0002/msk/rdb.ecommerce.customer/year={last_bookmark_time.year}/month={last_bookmark_time.month:02}/day={last_bookmark_time.day:02}/hour={last_bookmark_time.hour:02}'
        next_s3_path = f's3a://{bucket_name}/dms/ecommerce/customer/{cdc_partition_time.year}/{cdc_partition_time.month:02}/{cdc_partition_time.day:02}/{cdc_partition_time.hour:02}'
...
# """).show()
    ...
    FROM (
        SELECT
            a.*,
            ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY last_update_time DESC) as row_num
        FROM v_cdc_source a
